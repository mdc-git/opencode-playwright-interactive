...
After `node_repl_reset`, forced kernel termination, or a tool result reporting that the kernel restarted, rerun setup and the complete startup block before interacting again. Cancellation is not rollback: assignments, browser actions, requests, filesystem writes, and lexical declarations from before interruption may remain. Do not blindly replay a consequential action after cancellation; inspect its result first.
