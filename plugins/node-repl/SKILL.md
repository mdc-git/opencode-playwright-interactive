...
description: Persistent Playwright browser and Electron QA through node_repl, with optional humanized input for remote websites.
---

# Playwright Interactive

Use persistent `node_repl` Playwright handles for browser and Electron QA.
...
All REPL controls are native OpenCode tools. The agent **MUST** call them directly and **MUST NOT** wrap them in `execute`, `tools.node_repl`, a string, or a template literal.

- Setup with `node_repl_playwright_setup({})`.
- Run each cell with `node_repl({ code: 'plain Node.js source' })`.
- Manage jobs with `node_repl_job({ action: 'wait', id: 'repl_1' })`, `status`, `cancel`, or `list`.
- Reset with `node_repl_reset({})` only when losing bindings and browser handles is acceptable.
...
```js
node_repl_playwright_setup({})
```

### REQUIRED: Sequential REPL Calls

After setup, the agent **MUST** send each browser REPL cell as plain JavaScript in the `code` input of a direct `node_repl` call. The plugin executes every cell in the same persistent session kernel.

Browser startup, inspection, and interaction cells do not need an inner tool call.

Long cells require no wrapper or timeout option. If `node_repl` returns a job ID, use `node_repl_job` with `wait` when no other work is available, `status` for an immediate snapshot, and `cancel` to request cancellation. Do not submit another JavaScript cell while the kernel reports an active job. Long-running cells have no automatic wall-clock limit and may remain as background jobs while they make progress; the foreground handoff occurs after 35 seconds. Cancellation is cooperative when possible. The controller waits two seconds after requesting cancellation, then allows five more seconds before beginning the termination sequence if native work remains stuck. `wait` only observes a job and does not cancel a native promise. If native work does not return, cancellation can force termination of the session kernel; this loses that session's bindings and browser handles but does not restart OpenCode or affect other sessions. The completed job reports whether the kernel was preserved or terminated; after forced kernel loss, the next execution reports the restart and must rerun setup and the complete startup block. Use `node_repl_reset` only when losing bindings and browser handles is acceptable. Each `wait` observes for up to five seconds and does not stop or restart the job; if it still reports an active state, call `wait` again when no other work is available.

If `node_repl` reports a parse error, correct the Node.js source and retry the same small cell without resetting the kernel.
...
Humanized typing is intentionally slower. Long input cells automatically continue as background jobs when they exceed the 35-second foreground window; use `node_repl_job` to wait for or inspect them.
...
After `node_repl_reset`, forced kernel termination, or a tool result reporting that the kernel restarted, rerun setup and the complete startup block before interacting again. Cancellation is not rollback: assignments, browser actions, requests, and filesystem writes may remain after either outcome, while lexical declarations may remain when a preserved kernel was interrupted. Do not blindly replay a consequential action after cancellation; inspect its result first.
