...
function applyToolTransform(
  ...
  tools.add({
    name: 'js_repl_job',
    description:
      'Inspect, wait briefly for, or cancel controller-managed JavaScript REPL jobs. Job actions remain responsive while the session kernel is busy.',
    input: jobInput,
    output: textOutput,
    options: { permission: 'js_repl' },
