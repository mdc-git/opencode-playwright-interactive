...
license: Apache-2.0
compatibility: OpenCode with the project-local js_repl plugin and Node.js 22.22 or newer
metadata:
...
- The `js_repl` and `js_repl_reset` tools must be available. If they are missing, stop and tell the user to restart OpenCode from this project so `.opencode/plugins/js-repl.ts` loads.
...
First inspect the workspace. Do not create a new `package.json` in an existing non-Node project without asking the user.

```sh
test -f package.json || npm init -y
npm install playwright
# Required for Chromium web testing when the browser is not already installed:
npx playwright install chromium
# Electron only, and only when the target app does not already depend on it:
npm install --save-dev electron
node -e "import('playwright').then(() => console.log('playwright import ok')).catch((error) => { console.error(error); process.exit(1); })"
```

Repeat setup after switching to a different workspace. Browser packages and browser binaries are separate; a successful npm install does not guarantee Chromium is installed. Installing Linux system dependencies with `playwright install --with-deps` may require user approval and elevated privileges.
...
try {
  ...
  throw new Error(
    `Could not load playwright from the current js_repl workspace. Run setup here first. Original error: ${error}`
  );
}
```
...
- `Cannot find module 'playwright'`: install it in the current application workspace and verify the import through `js_repl`.
- Browser executable missing: run `npx playwright install chromium` in that workspace.
