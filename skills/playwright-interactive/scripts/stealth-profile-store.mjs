...
import { BEHAVIOR_SCHEMA, MOBILE_DEVICE_SCALE_FACTOR, PERSONA_SCHEMA, clamp, uniform } from "./stealth-utils.mjs";
...
export const createBehavior = () => normalizeBehavior({
  ...defaults,
...
export const normalizeMetadata = (value) => value && [1, 2].includes(value.schema) && typeof value.profileId === "string" && value.profileId
...
export function createProfileStore(baseDir) {
  ...
  const writeRecord = async (file, value, maximum = DEFAULT_MAXIMUM) => {
    const serialized = JSON.stringify(value, null, 2);
    if (Buffer.byteLength(serialized) > maximum) throw new Error(`Stealth record is unexpectedly large: ${file}`);
    const write = async () => {
  ...
  const quarantine = async (file, reason) => {
    ...
    console.warn(`Stealth profile record ${file} ${reason}; moving it to ${recovered} and creating a fresh one.`);
  ...
  const readRecord = async (file, normalize, create, maximum = DEFAULT_MAXIMUM) => {
  ...
  const loadBehavior = ({ dataDir } = {}) => readRecord(paths({ dataDir }).behaviorProfile, normalizeBehavior, createBehavior);
